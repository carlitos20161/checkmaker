...
const [userLoaded, setUserLoaded] = useState(false);
  ...
  const handlePrintReviewedChecks = async (companyId: string | null, weekKey: string | null) => {
    ...
    try {
      const response = await fetch('http://localhost:5004/api/print_selected_checks', {
        method: 'POST',
  ...
  const handleDirectReviewAll = async () => {
    ...
    try {
      ...
      setReviewOnly(false);
      alert('All checks marked as reviewed!');
    } catch (err) {
                ...
                <>
                  ...
                  <Button
                    ...
                    color="primary"
                      ...
                      {/* For non-admin users: Send all for review */}
                      {currentRole !== 'admin' && (
                        <Button
                          variant="contained"
                          color="warning"
                          onClick={() => handleBulkMarkAsReviewed(selectedCompanyId, selectedWeekKey)}
                    disabled={!selectedCompanyId || !selectedWeekKey}
                          sx={{ ml: 1 }}
                  >
            ...
            <>
              ...
              <Box sx={{ mt: 2, p: 2, backgroundColor: '#f8f9fa', borderRadius: 1, border: '1px solid #e9ecef' }}>
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                              ...
                              {(() => {
                                // Get the hours for this specific relationship
                                ...
                                if (relHours > 0) {
                                  return (
                                    <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                                      <span>Hours:</span>
                                      <span>{relHours}h × ${(rel.payRate || parseFloat(selectedCheck.payRate?.toString() || '0')).toFixed(2)} = ${(relHours * (rel.payRate || parseFloat(selectedCheck.payRate?.toString() || '0'))).toFixed(2)}</span>
                                    </Box>
                                  );
                                }
                                
                                // If no relationship hours, show a fallback message
                                return (
                                  <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                                    <span>Hours:</span>
                  ...
                  {selectedCheck.otHours && selectedCheck.otHours > 0 && (
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                              ...
                              {(() => {
                                ...
                                if (!hasDailyAmounts) {
                                  console.log('🔍 DEBUG perDiem: No daily amounts found, returning null');
                                  return null;
                                }
